...
# 1. Định nghĩa HTML lý thuyết mới hoàn chỉnh cho Chuyên đề 6
...
# Định vị thẻ theory-content
ts_html = content.find('<div class="theory-text" id="theory-content"')
if ts_html < 0:
    print("❌ Không tìm thấy theory-content")
    sys.exit(1)

# Định vị thẻ <script> tiếp theo sau theory-content
te_script = content.find('<script>', ts_html)
if te_script < 0:
    print("❌ Không tìm thấy thẻ <script>")
    sys.exit(1)

# Thay thế từ ts_html cho đến trước te_script
# Cần đóng 2 div ngoài cùng: modal-content và theoryModal
full_replacement = new_html_theory + "\n        </div>\n    </div>\n\n    "

content_new = content[:ts_html] + full_replacement + content[te_script:]

# Lưu file
with open(html_path, 'w', encoding='utf-8') as f:
    f.write(content_new)

print("✅ Đã cập nhật lại lý thuyết và đóng div chuẩn xác!")
