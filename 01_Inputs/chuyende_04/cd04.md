...
- Đồ thị hàm số có đỉnh là $I\left(x_{I} ; y_{I}\right) \Leftrightarrow\left\{\begin{array}{l}-\frac{b}{2 a}=x_{I} \\ -\frac{\Delta}{4 a}=y_{I}\end{array}\left(\left\{\begin{array}{c}-\frac{b}{2 a}=x_{I} \\ f\left(x_{I}\right)=y_{I}\end{array}\right)\right.\right.$.
- Trên $\mathbb{R}$, ta có:

1. $f(x)$ có giá trị lớn nhất $\Leftrightarrow a<0$. Lúc này $\operatorname{Max}_{\mathbb{R}} f(x)=-\frac{\Delta}{4 a}=f\left(-\frac{b}{2 a}\right)$.
2. $f(x)$ có giá trị nhỏ nhất $\Leftrightarrow a>0$. Lúc này $\operatorname{Min}_{\mathbb{R}} f(x)=-\frac{\Delta}{4 a}=f\left(-\frac{b}{2 a}\right)$.

## 3. DẤU CỦA TAM THỨC BẬC HAI

Tam thức bậc hai (đối với $x$ ) là biểu thức có dạng $a x^{2}+b x+c$, trong đớ $a, b, c$ là những số thực cho trước $(a \neq 0)$, được gọi là các hệ số của tam thức bậc hai.
...
Nếu $\Delta=0$ thì $f(x)$ cùng dấu với hệ số $a$ và với mọi $x \neq-\frac{b}{2 a}$ và $f\left(-\frac{b}{2 a}\right)=0$.
Nếu $\Delta>0$ thì tam thức $f(x)$ có hai nghiệm phân biệt ${ }^{x_{1}}$ và $x_{2}\left(x_{1}<x_{2}\right)$. Khi đó, $f(x)$ cùng dấu với hệ số $a$ với mọi $x \in\left(-\infty ; x_{1}\right) \cup\left(x_{2} ;+\infty\right)$; $f(x)$ trái dấu với hện số a với mọi $x \in\left(x_{1} ; x_{2}\right)$.

Nhớnhanh. Khi $\Delta>0$, dấu của ${ }^{f(x)}$ và a là: "Trong trái, ngoài cùng"
...
Chúý. Trong định lí về tam thức bậc hai có thể thay $\Delta$ bởi $\Delta^{\prime}$.

## 4. PHƯƠNG TRÌNH QUY VỀ PHƯƠNG TRÌNH BẬC HAI

Để giải phương trình $\sqrt{a x^{2}+b x+c}=\sqrt{d x^{2}+e x+f}$, ta thực hiện nhưs sau:
...
Câu 1. Hàm số $y=a x^{2}+b x+c,(a>0)$ đồng biến trong khoảng nào sau đậy?
...
Hoành độ của đỉnh là $x_{I}=-\frac{b}{2 a}=1$ nên ta loại A và chọnD.
...
Cau 7. Bảng biến thiên ở dưới là bảng biến thiên của hàm số nào trong các hàm số được cho ở bốn phương án $\mathrm{A}, \mathrm{B}, \mathrm{C}, \mathrm{D}$ sau đây?
...
$\Rightarrow\left\{\begin{array}{l}\frac{-b}{2 a}=\frac{5}{2} \\ \frac{-\Delta}{4 a}=\frac{1}{2} \\ a+b+c=-4\end{array} \Leftrightarrow\left\{\begin{array}{l}\frac{-b}{2 a}=\frac{5}{2} \\ \frac{-b^{2}+4 \mathrm{a} c}{4 a}=\frac{1}{2} \\ a+b+c=-4\end{array} \Leftrightarrow\left\{\begin{array}{l}b=-5 \mathrm{a} \\ \frac{-25 a^{2}+4 \mathrm{a}(4 \mathrm{a}-4)}{4 a} \\ c=4 \mathrm{a}-4\end{array}=\frac{1}{2} \Leftrightarrow\left\{\begin{array}{l}a=-2 \\ b=10 \\ c=-12\end{array}\right.\right.\right.\right.$
...
Câu 1. Số nghiệm nguyên của phương trình sau $\sqrt{x+3}-\sqrt{2 x-1}=1$ là:
A. 0 .
B. 2 .
C. 1 .
D. 3 .
